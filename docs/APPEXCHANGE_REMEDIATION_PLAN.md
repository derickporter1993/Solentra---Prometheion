# PROMETHEION APPEXCHANGE REMEDIATION PLAN
...
| `PrometheionComplianceScorer.cls` | 3 | Returns `Decimal` but callers expect `ScoreResult` | Add inner class with `frameworkScores` Map and `overallScore` Decimal |
...
| File | Line | Issue | Fix |
|------|------|-------|-----|
| `PrometheionGraphIndexer.cls` | 51 | `generateDeterministicHash()` uses `System.now().getTime()` | Remove timestamp from hash input |
...
|----|------|-------|-------|-----|
| S1 | `PrometheionQuickActionsService.cls` | 63-75, 104-111, 161-168, 228-233, 274-281, 380-385, 422-428, 469-473 | 8 queries missing `WITH SECURITY_ENFORCED` | Add clause to all queries |
| S2 | `PrometheionQuickActionsService.cls` | 139, 236, 245, 338, 393, 441, 475 | 7 DML operations without CRUD check | Use `Security.stripInaccessible()` or validate permissions |
...
| ID | File | Lines | Issue | Fix |
|----|------|-------|-------|-----|
| S6 | `PrometheionScoreCallback.cls` | 14-15 | REST endpoint has NO authentication | Add API key or JWT validation |
| S7 | `PrometheionScoreCallback.cls` | 102-109 | Exposes stack traces to external callers | Remove `e.getStackTraceString()` from response |
...
|----|-----------|--------------|---------------|
| T1 | `PrometheionQuickActionsServiceTest.cls` | PrometheionQuickActionsService | >75% |
| T2 | `PrometheionGraphIndexerTest.cls` | PrometheionGraphIndexer | >75% |
| T3 | `PrometheionGDPRDataErasureServiceTest.cls` | PrometheionGDPRDataErasureService | >75% |
| T4 | `PrometheionScoreCallbackTest.cls` | PrometheionScoreCallback | >75% |
...
| ID | File | Lines | Issue | Fix |
|----|------|-------|-------|-----|
| TF1 | `PrometheionEventPublisherTest.cls` | 21, 37, 51, 74, 84 | `System.assert(true)` - meaningless assertions | Add actual verification logic |
...
| TR3 | `PrometheionPCIAccessAlertTrigger.trigger` | 70+ lines of logic in trigger | Move to handler |
| TR4 | `PrometheionAlertTrigger.trigger` | No handler class | Create handler, delegate logic |
...
| E2 | `prometheionAiSettings.html` | 8-12, 21-25, 34-37, 50-54, 66-69, 81-85 | Malformed `onchange` handlers | Fix syntax |
| E3 | `prometheionReadinessScore.html` | 54-58, 65-69 | Malformed `onclick` handlers | Fix syntax |

#### 1.8 Add ARIA Labels & Accessibility (6 hours)

| ID | Component | Issue | Fix |
|----|-----------|-------|-----|
| A1 | `prometheionCopilot.html` | SVG icons missing `aria-hidden` | Add `aria-hidden="true"` to decorative icons |
| A2 | `riskHeatmap.html` | Color-only severity indicators | Add text labels or ARIA descriptions |
| A3 | `prometheionROICalculator.html` | Form inputs not properly labeled | Use component's `label` attribute |
...
- `riskHeatmap.html`
- `prometheionAiSettings.html`
- `prometheionReadinessScore.html`
- `prometheionROICalculator.html`
...
| J2 | `prometheionDrillDownViewer.js` | 29, 69, 86 | `JSON.parse` without try-catch | Wrap in try-catch |
...
| ID | File | Issue | Fix |
|----|------|-------|-----|
| SCH1 | `PrometheionCCPASLAMonitorScheduler.cls` | No try-catch in execute() | Add comprehensive error handling |
| SCH2 | `PrometheionDormantAccountAlertScheduler.cls` | No error handling in 4 methods | Add try-catch to all methods |
| SCH3 | `PrometheionGLBAAnnualNoticeScheduler.cls` | No error handling, no logging | Add both |
| SCH4 | `PrometheionGLBAAnnualNoticeBatch.cls` | No error handling in start/finish | Add error handling |
| SCH5 | `PrometheionGLBAAnnualNoticeBatch.cls` | Line 100 - Email commented out | Uncomment or implement alternative |
| SCH6 | `PrometheionGLBAAnnualNoticeBatch.cls` | Line 63 - No SaveResult handling | Implement `Database.SaveResult[]` handling |

#### 2.2 Permission Set Fixes (4 hours)

| ID | Issue | Fix |
|----|-------|-----|
| P1 | `Prometheion_Admin_Extended` has `modifyAllRecords=true` on all objects | Remove or restrict |
...
| Q3 | `PrometheionISO27001AccessReviewService.cls` | 309-378 | 6 separate COUNT queries | Combine into aggregate query |
| Q4 | `PrometheionGDPRDataErasureService.cls` | 52-128 | No savepoint/rollback | Add Database.Savepoint |

#### 2.4 Sharing Declaration Fixes (2 hours)

| File | Current | Should Be |
|------|---------|-----------|
| `PrometheionAuditTrailPoller.cls` | No declaration | `without sharing` (schedulable) |
| `PrometheionScoreCallback.cls` | No declaration | `without sharing` (external API) |
...
| Duplicate scheduler classes | `ISO27001QuarterlyReviewScheduler` vs `PrometheionISO27001QuarterlyScheduler` | Remove duplicate |
...
| File | Lines | Issue | Fix |
|------|-------|-------|-----|
| `prometheionCopilot.html` | 103, 124 | User content in `lightning-formatted-rich-text` | Sanitize server-side or use `lightning-formatted-text` |
...
4. `PrometheionComplianceScorer.cls:3` - Return type mismatch
5. `ComplianceTestDataFactory.cls:105` - Picklist case mismatch
6. `PrometheionGraphIndexer.cls:51` - Non-deterministic hash
7. `PrometheionScoreCallback.cls:14-15` - No authentication
8. `PrometheionScoreCallback.cls:102-109` - Stack trace exposure
...
12. `PrometheionCCPASLAMonitorScheduler.cls:22` - No error handling
13. `PrometheionDormantAccountAlertScheduler.cls` - No error handling (4 methods)
14. `PrometheionGLBAAnnualNoticeScheduler.cls:19` - No error handling, no logging
15. `PrometheionGLBAAnnualNoticeBatch.cls:100` - Email commented out
16. `Prometheion_Admin_Extended.permissionset` - modifyAllRecords=true
17. `systemMonitorDashboard.html:29-33` - Malformed onclick
18. `prometheionAiSettings.html` - 6 malformed onchange handlers
