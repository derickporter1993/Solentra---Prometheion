...
export default class PrometheionTrendAnalyzer extends LightningElement {
  ...
  wiredDateFields({ error, data }) {
    if (data) {
      ...
      this.showError(
        "Error loading date fields: " + (error.body ? error.body.message : error.message)
      );
    }
  }
  ...
  wiredMetricFields({ error, data }) {
    if (data) {
      ...
      this.showError(
        "Error loading metric fields: " + (error.body ? error.body.message : error.message)
      );
    }
  }
  ...
  handleAnalyze() {
    ...
    getTimeSeries({
      ...
      .catch((error) => {
        this.hasError = true;
        this.errorMessage =
          "Error analyzing trend: " + (error.body ? error.body.message : error.message);
        this.isLoading = false;
