 ...
 *
 * Listens for PCI Access Events via Prometheion_Raw_Event__e and triggers alerts for:
 * - Failed access attempts
 * - Access to sensitive data outside business hours
 * - Bulk data access patterns
 *
 * Note: Uses generic Prometheion_Raw_Event__e filtered by Event_Type__c = 'PCI_ACCESS'
...
trigger PrometheionPCIAccessAlertTrigger on Prometheion_Raw_Event__e (after insert) {
    if (!TriggerRecursionGuard.isFirstRun('PrometheionPCIAccessAlertTrigger')) return;

    try {
        List<Prometheion_Raw_Event__e> pciEvents = new List<Prometheion_Raw_Event__e>();

        // Filter for PCI_ACCESS events only
        for (Prometheion_Raw_Event__e event : Trigger.new) {
        ...
        List<Prometheion_Raw_Event__e> failedAccessEvents = new List<Prometheion_Raw_Event__e>();
        List<Prometheion_Raw_Event__e> afterHoursEvents = new List<Prometheion_Raw_Event__e>();
        List<Prometheion_Raw_Event__e> bulkAccessEvents = new List<Prometheion_Raw_Event__e>();

        // Track access counts by user for bulk detection
        Map<String, Integer> accessCountByUser = new Map<String, Integer>();

        for (Prometheion_Raw_Event__e event : pciEvents) {
            // Parse JSON payload
